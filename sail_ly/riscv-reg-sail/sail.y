...
%%   
id: ID {
        if(strstr($1.stringVal, "_flag") || strstr($1.stringVal, "_vmask")){
                regname = $1.stringVal;    
        }
        if(funName==1 && strstr($1.stringVal, "reg")){
                regname = $1.stringVal;
                funName = 0;
        }
        if(regCount == 1 && funCount == 1){
                if(strstr($1.stringVal, "x") || strstr($1.stringVal, "zero_reg")){
                        //cout << $1.stringVal << endl;
                        regstr+=$1.stringVal; 
                        regstr+=","; 
                }
                if(strstr($1.stringVal, "f")){
                        //cout << $1.stringVal << endl;
                        regstr+=$1.stringVal; 
                        regstr+=","; 
                }
        ...
        if(boolbits == 1 || sizebits == 1 || encdec_rounding_mode == 1 || Mext_U == 1){  
        ...
        else if(strstr($1.stringVal, "encdec_") && mapop < 1 && instr_bits < 1){
                mapop++;  
                if(mapop == 1){  
                        oplistcheck = 1;  
                }
               ...
               // printf("\n1%s\n",$1.stringVal); 
                ...
                else if(strstr($1.stringVal,"encdec_")){
                        BitStr=BitStr+"op";
                        Getencdecop = 1;
...
exp: exp0
   ...
   | If_ exp Then exp
   | Match { regCount = 1; } exp '{' case_list '}' 
   { 
...
fun_def: Function_ { funName = 1; funCount = 1; }funcls 
...
map_def: Mapping ID{
                ...
                if(strstr($2.stringVal, "regtype") || strstr($2.stringVal, "reg_")){
                        regname.clear();
                        regname = $2.stringVal;
                        funName = 0;
                        regCount = 1;
        ...
        | Mapping id ':' typschm '=' '{'{regFlag=3;} mapcl_list '}' {
                funName =0;
                funCount=0;
                regCount=0;
                regFlag = 0;
...
scattered_def: Scattered Enum id
                ...
                | Enum Clause id '=' id
                | Function_ Clause funcl { regstr.clear();}
                | Union {
                        if(assemblyString == 3){ 
